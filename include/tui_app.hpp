...
class TuiApp {
public:
    static void run(TelemetryRingBuffer& ring_buffer);
};
...
void TuiApp::run(TelemetryRingBuffer& ring_buffer) {
    ...
    // Component block to display telemetry variables
    ...
    // Capture standard Vim keys for scroll layout selections
    auto input_handler = CatchEvent(renderer, [&](Event event) {
        ...
        if (event == Event::Character('k')) {
            if (selected_layer > 0) selected_layer--;
            return true;
        }
    ...
    // Create a background UI tick loop so information automatically refreshes
